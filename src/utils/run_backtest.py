...
def run_backtest(config_path):
    ...
    pyfolio_enabled = bool((config.get('backtesting', {}) or {}).get('enable_pyfolio', False))
    engine = BacktestEngine(
        initial_cash=config['backtesting']['initial_cash'],
        commission=config['backtesting']['commission'],
        enable_pyfolio=pyfolio_enabled,
    )
    ...
    # If PyFolio is enabled, persist its artifacts to CSV for CLI usage
    try:
        if pyfolio_enabled:
            symbols_str = ",".join(symbols) if symbols else ""
            result_key = f"{engine._strategy_class.__name__}_{symbols_str}"
            result_obj = engine.results.get(result_key, {})
            if result_obj.get('has_pyfolio'):
                pf_out_dir = Path(config['backtesting'].get('output_dir', 'results'))
                pf_out_dir.mkdir(parents=True, exist_ok=True)

                returns = result_obj.get('pf_returns')
                positions = result_obj.get('pf_positions')
                transactions = result_obj.get('pf_transactions')
                gross_lev = result_obj.get('pf_gross_lev')

                try:
                    if returns is not None:
                        returns.to_csv(pf_out_dir / f"{strategy_name}_pyfolio_returns.csv")
                    if positions is not None:
                        positions.to_csv(pf_out_dir / f"{strategy_name}_pyfolio_positions.csv")
                    if transactions is not None:
                        transactions.to_csv(pf_out_dir / f"{strategy_name}_pyfolio_transactions.csv")
                    if gross_lev is not None:
                        gross_lev.to_csv(pf_out_dir / f"{strategy_name}_pyfolio_gross_lev.csv")
                    print(f"Saved PyFolio artifacts to {pf_out_dir}")
                except Exception as e:
                    print(f"Warning: failed to save PyFolio artifacts: {e}")
    except Exception as e:
        print(f"Warning: PyFolio export step failed: {e}")
